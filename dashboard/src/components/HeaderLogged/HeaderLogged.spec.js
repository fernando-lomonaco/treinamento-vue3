...
describe('<HeaderLogged />', () => {
  ...
  it.skip('should render 3 dots where there\'s not user logged', async () => {
    router.push('/')
    await router.isReady()
    const wrapper = shallowMount(HeaderLogged, {
      global: {
  ...
  it.skip('should render 3 dots where there\'s user logged', async () => {
    router.push('/')
    await router.isReady()
    mockStore.currentUser.name = 'Fernando'
    const wrapper = shallowMount(HeaderLogged, {
      global: {
        plugins: [router]
      }
    ...
    const buttonLogout = wrapper.find('#logout-button')
    expect(buttonLogout.text()).toBe('Fernando (sair)')
